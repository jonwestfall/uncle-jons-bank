...
router = APIRouter(
    prefix="/tests",
    tags=["NON-PROD: Test Utilities"],
    dependencies=[Depends(require_role("admin"))],
)


@router.post("/run", summary="NON-PROD: Run integration seed/test flow")
...
@router.post(
    "/interest-test", summary="NON-PROD: Run interest accrual simulation test"
)
...
@router.post("/cd-issue", summary="NON-PROD: Seed and issue a sample CD")
...
@router.post("/cd-redeem", summary="NON-PROD: Force CD redeem test helper")
async def cd_redeem_route(
    cd_id: int,
    persist: bool = False,
):
    """Redeem a CD as if matured today."""
